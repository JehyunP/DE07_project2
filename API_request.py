...
class API_Request:
    ...
    def request_api_marine_depth(self, pgn='1', nor='10', dtype='JSON', year='2025', month='08', station='955'):
        '''
            Request the observation data of station at year-month

            param :
                pgn : number of page request (default = 1)
                nor : number of rows -> number of info in a page (default = 10)
                dtype : the data type of API output (XML / JSON)
                year : the year of request data
                month : the month of request data
                station : Specific stn id 
        '''

        # URL of Endpoint
        url = 'https://apihub.kma.go.kr/api/typ02/openApi/SeaMtlyInfoService/getDailyLhaws?'
        pgn_ = f'pageNo={pgn}&'
        nor_ = f'numOfRows={nor}&'
        dtype_ = f'dataType={dtype}&'
        year_ = f'year={year}&'
        month_ = f'month={month}&'
        station_ = f'station={station}&'
        auth_key = f'authKey={self.api_key}'
        request_url = url + pgn_ + nor_ + dtype_ + year_ + month_ + station_ + auth_key
        ...
        try:
            ...
            data = response.json()
            info_list = data['response']['body']['items']['item'][0]['day']['info']
            day_info = data['response']['body']['items']['item'][0]['day']

            df = pd.DataFrame(info_list)
            df["stn_ko"] = day_info.get("stn_ko")
            df["stn_en"] = day_info.get("stn_en")
            df["stn_id"] = day_info.get("stn_id")
            df['year'] = int(year)
            df['month'] = int(month)
            df.replace(['null', 'x', ' ', ''], np.nan, inplace=True)
            cols = ['ls', 'ls_']

            
            df = df[~df['tm'].isin(['상순', '중순', '하순'])]

            if df[cols].isna().all().all():
                print(f"Depth info not included for {day_info.get('stn_ko')} ({year}-{month})")
                return False
            
            df.to_csv(f'data/marine_depth_condition/{day_info.get("stn_ko")}_{year}{month}.csv',
    ...
    def request_api_marine_depth_loop(self, file, from_year='2025', from_month='07', to_year='2025', to_month='08'):
        '''
            Run loop to request all data within given period
            The process would be requesting api from each station from file.csv 

            param :
                file : the stn_marine_depth_info csv's local directory -> use for read 
                from/to year, month : each initial period of year and month to end period
        '''
        
        info_df = pd.read_csv(file)

        for id in info_df['stn_id']:
            # Will be used for time calculation
            start = time.time()

            start_date = datetime(int(from_year), int(from_month), 1)
            end_date = datetime(int(to_year), int(to_month), 1)
            ...
            while start_date <= end_date:
                ...
                print(f'Requesting {y}-{m}')

                try:
                    self.request_api_marine_depth(year=str(y), month=f"{m:02d}", station=str(id))
                except Exception as e:
                    print(f'Fail requesting at {y}-{m}')
            ...
            # Calculate time spent
            elapse = end - start
            print(f'Total Run time of {id} : {elapse: .2f} sec')
    ...
    def __init__(self):
        ...
        self.request_api_marine_depth_loop(
            os.getenv('STN_MARINE_DEPTH_FILE_PATH'),
                    from_year = '2016',
                    from_month = '09'
        )
        #self.request_api_marine_depth()
